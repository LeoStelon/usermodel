	...
	try {
		...
		if (user) {
			...
			const msg = {
				...
				html: `<strong>Please Click the link below to reset your password, the link is valid only for 1 hour!</strong><br><a href="http://localhost:3000/forgotpassword.html?token=${token}">Reset Password<a/>`,
			};
			sgMail
				.send(msg)
				.then(() => {
		...
		res.send({message:"Email has been sent to the associated mail address you provided if it exist in our database."});
	} catch (e) {
		res.status(500).send();
	}
});
